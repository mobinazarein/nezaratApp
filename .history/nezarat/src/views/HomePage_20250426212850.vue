            ...
            <tbody>
              <tr v-for="item in sortedItems" :key="item.id">
                <td v-for="(col, index) in columns" :key="index"
                  style="background-color: #FFFFFF; border: 1px solid #0000001A;">
                  <template v-if="col.value === 'operation'">
                    <ion-button fill="clear" size="small" color="primary" @click="editItem(item)">
                      <img src="../assets/Clipboard.svg" /> {{ item[col.value] }}
                    </ion-button>
                  </template>

                  <template v-else-if="col.value === 'drillingNumber'">
                    {{ item[col.value] }}
                    <ion-icon name="eye-outline" style="margin-right: 8px; font-size: 18px;"></ion-icon>
                  </template>
...
const sortedItems = computed(() => {
  let filtered = Items.value.filter(i => i.title.includes(searchQuery.value) || i.drillingNumber.includes(searchQuery.value));
  if (sortField.value) {
    filtered.sort((a, b) => {
