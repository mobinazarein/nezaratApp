                  ...
                  <template v-if="col.sortable">
                    <div @click="sortBy(col.value)" style="cursor: pointer; display: flex; align-items: center;">
                      ...
                      <img v-if="sortField === col.value"
                        :src="sortOrder === 'asc' ? require('../assets/sort-up.png') : require('../assets/sort-down.png')"
                        style="width: 20px; height: 20px; margin-left: 4px;" />
                      <img v-else :src="require('../assets/sort-default.png')"
                        style="width: 20px; height: 20px; margin-left: 4px;" />
                    </div>
                  </template>
                  <template v-else>
...
const sortedItems = computed(() => {
  ...
  if (sortField.value === 'drillingNumber') {  // فقط برای drillingNumber سورت انجام می‌شود
    filtered.sort((a, b) => {
      let aValue = a[sortField.value];
      let bValue = b[sortField.value];

      // تبدیل به عدد برای سورت کردن به درستی
